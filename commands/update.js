...
export default {
  ...
  async execute(msg, { sock, args, isOwner, settings }) {
    ...
    try {
      ...
      // Auto-restart after successful update without killing the terminal
      setTimeout(() => {
        console.log('[UPDATE] Auto-restarting bot after successful update...');
        // Use PM2 restart if available, otherwise graceful restart
        exec('pm2 restart index.js', (error) => {
          if (error) {
            // Fallback to process restart
            console.log('[UPDATE] PM2 not available, performing process restart...');
            process.exit(0);
          }
        });
      }, 3000);
