    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
      for (const msg of messages) {
        try {
          ...
          // Anti-detection for groups (optimized - single metadata call)
          if (isGroup && !isNewsletter && !isFromMe && (global.antiLinkWarn[remoteJid] || global.antiLinkKick[remoteJid] || global.antiBadWord[remoteJid])) {
            let groupMeta = null;
            let botIsAdmin = false;
            let senderIsAdmin = false;
            
            // Anti-link detection
            if (global.antiLinkWarn[remoteJid] || global.antiLinkKick[remoteJid]) {
              const linkRegex = /(https?:\/\/[^\s]+|www\.[^\s]+|[^\s]+\.[^\s]{2,})/gi;
              if (linkRegex.test(body)) {
                try {
                  groupMeta = await sock.groupMetadata(remoteJid);
                  botIsAdmin = groupMeta.participants.find(p => p.id === sock.user.id && p.admin);
                  senderIsAdmin = groupMeta.participants.find(p => p.id === senderJid && p.admin);
            ...
            // Anti-badword detection (reuse metadata if already fetched)
